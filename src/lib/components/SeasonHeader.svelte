...
<div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl space-y-6">
  ...
  <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
    ...
    <div class="space-y-1">
      <div class="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-lg bg-slate-950 border border-slate-800 text-indigo-400 text-[11px] font-bold uppercase tracking-wider">
  ...
  <!-- Executive Financial & Champions Banner -->
  {#if $currentSeasonSummary}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      
      <!-- Card 1: Dimensão e Jogos da Temporada -->
      <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-1">
        <div class="flex items-center justify-between text-slate-400 text-xs font-semibold">
          <span class="flex items-center gap-1.5 text-indigo-300 font-bold">
            <Calendar class="w-4 h-4 text-indigo-400" /> Total de Jogos
          </span>
          <span class="text-[10px] text-slate-500 font-mono">T{$currentSeasonNum}</span>
        </div>
        <p class="text-lg font-black text-white font-mono">
          {((($currentSeasonSummary.financeiro?.total_jogos_c || 0) + ($currentSeasonSummary.financeiro?.total_jogos_d || 0))).toLocaleString('pt-BR')} <span class="text-xs font-normal text-slate-400">partidas</span>
        </p>
        <div class="flex items-center justify-between text-[10px] text-slate-400 pt-0.5">
          <span class="font-bold text-slate-300">204 clubes em disputa</span>
      ...
      <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-1">
        <div class="flex items-center justify-between text-slate-400 text-xs font-semibold">
          <span>Orçamento Logístico Pirâmide</span>
          <DollarSign class="w-4 h-4 text-emerald-400" />
        </div>
        <p class="text-lg font-black text-emerald-300 font-mono">
        ...
        <div class="flex items-center justify-between text-[10px] text-slate-400 pt-0.5">
          ...
          <span>D: {formatMoney($currentSeasonSummary.financeiro?.custo_total_d_brl)}</span>
        </div>
      </div>
      ...
      <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-1">
        <div class="flex items-center justify-between text-slate-400 text-xs font-semibold">
          <span>Economia de Turnês (TTP)</span>
        ...
        <span class="text-[10px] text-slate-400 block pt-0.5">
          Economia direta ao agrupar jogos fora em turnê sequencial
        </span>
      </div>
      ...
      <div class="bg-slate-950/70 border border-slate-800 rounded-xl p-4 space-y-2">
        <div class="flex items-center justify-between text-slate-400 text-xs font-semibold">
          <span>Matriz de Modais</span>
          <Bus class="w-4 h-4 text-cyan-400" />
        </div>

        <div class="space-y-1 pt-0.5">
          <div class="flex justify-between text-[11px] font-mono">
            <span class="text-slate-400">Série C (Ônibus):</span>
          ...
          <div class="flex justify-between text-[11px] font-mono pt-1">
            <span class="text-slate-400">Série D (Ônibus):</span>
